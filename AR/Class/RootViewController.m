...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        NSString *machineInfoShort = [[machineInfo substringToIndex:7] lowercaseString];
        ...
        } else if ([machineInfoShort isEqualToString:@"iphone7"]) {
            machineModelVersion = 3;
        } else {
...
- (void)loadView {
    ...
    CGRect camBaseFrame = CGRectMake(0, 0, screenRect.size.width, screenRect.size.height);
    camView = [[CamView alloc] initWithFrame:camBaseFrame];
    [baseView addSubview:camView];
    camView.backgroundColor = [UIColor blueColor];
    
    // create view for processed frames
    procFrameView = [[UIImageView alloc] initWithFrame:camBaseFrame];
    [procFrameView setHidden:YES];  // initially hidden
    procFrameView.backgroundColor = [UIColor yellowColor];
...
//    glView.backgroundColor = [UIColor brownColor];
    ...
    for (int btnIdx = 0; btnIdx < btnTitles.count; btnIdx++) {
        ...
        [procOutputSelectBtn setOpaque:NO]; //YES
...
//        [self interfaceOrientationChanged: (UIInterfaceOrientation) 4];
        NSLog(@"interfaceOrientation: %ld", (long)self.interfaceOrientation);
    }
...
- (void)interfaceOrientationChanged:(UIInterfaceOrientation)o {
    [[(AVCaptureVideoPreviewLayer *)camView.layer connection] setVideoOrientation:(AVCaptureVideoOrientation)(UIInterfaceOrientation) 3]; //o
}
